drop database university;

create database if not exists university;

use university;

create table faculty(id INT AUTO_INCREMENT, name VARCHAR(40) NOT NULL UNIQUE, recruitment INT NOT NULL, PRIMARY KEY (id));

create table subject(id INT AUTO_INCREMENT, name VARCHAR(40) NOT NULL UNIQUE, PRIMARY KEY (id));

create table faculty_subject(faculty INT NOT NULL, subject INT NOT NULL);

create table user(id INT AUTO_INCREMENT, login VARCHAR(40) NOT NULL UNIQUE, password VARCHAR(100) NOT NULL,
role ENUM('enrollee','committee') NOT NULL, name VARCHAR(40) NOT NULL, surname VARCHAR(40) NOT NULL, PRIMARY KEY (id));

create table certificate(id INT AUTO_INCREMENT, application INT NOT NULL, subject INT NOT NULL, mark INT NOT NULL, PRIMARY KEY (id));

create table application(id INT AUTO_INCREMENT, user INT NOT NULL, faculty INT NOT NULL, average_mark INT NOT NULL,
 status ENUM('applied', 'cancelled', 'registered', 'entered', 'unentered'), PRIMARY KEY (id));

insert into faculty (name, recruitment) values
( 'mathematical', 20),
( 'linguistic', 25),
( 'physical', 22),
( 'economic', 10),
( 'biological', 10),
( 'geographical', 10);

insert into subject (name) values
('language'),
('mathematics'),
('physics'),
('foreign_language'),
('chemistry'),
('biology'),
('geography');

insert into faculty_subject (faculty, subject) values
(1 , 1), (1 , 2), (1 , 3),
(2 , 1), (2 , 4), (2 , 2),
(3 , 1), (3 , 2), (3 , 3),
(4 , 1), (4 , 4), (4 , 2),
(5 , 1), (5 , 5), (5 , 6),
(6 , 1), (6 , 6), (6 , 7);

insert into user (login, password, role, name, surname) values 
( 'admin', 'd033e22ae348aeb5660fc2140aec35850c4da997', 'committee', 'Admin', 'Adminov'),
( 'user', '12dea96fec20593566ab75692c9949596833adc9', 'enrollee', 'Andrei', 'Andreev'),
( 'userA', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiA', 'Andreev'),
( 'userB', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiB', 'Andreev'),
( 'userC', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiC', 'Andreev'),
( 'userD', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiD', 'Andreev'),
( 'userE', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiE', 'Andreev'),
( 'userF', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiF', 'Andreev'),
( 'userG', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiG', 'Andreev'),
( 'userH', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiH', 'Andreev'),
( 'userI', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiI', 'Andreev'),
( 'userJ', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiJ', 'Andreev'),
( 'userK', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiK', 'Andreev'),
( 'userL', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiL', 'Andreev'),
( 'userM', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiM', 'Andreev'),
( 'userN', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiN', 'Andreev'),
( 'userO', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiO', 'Andreev'),
( 'userP', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiP', 'Andreev'),
( 'userQ', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiQ', 'Andreev'),
( 'userR', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiR', 'Andreev'),
( 'userS', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiS', 'Andreev'),
( 'userT', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiT', 'Andreev'),
( 'userU', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiU', 'Andreev'),
( 'userV', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiV', 'Andreev'),
( 'userW', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiW', 'Andreev'),
( 'userX', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiX', 'Andreev'),
( 'userY', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiY', 'Andreev'),
( 'userZ', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiZ', 'Andreev'),
( 'userAA', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiA', 'Andropov'),
( 'userBB', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiB', 'Andropov'),
( 'userCC', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiC', 'Andropov'),
( 'userDD', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiD', 'Andropov'),
( 'userEE', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiE', 'Andropov'),
( 'userFF', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiF', 'Andropov'),
( 'userGG', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiG', 'Andropov'),
( 'userHH', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiH', 'Andropov'),
( 'userII', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiI', 'Andropov'),
( 'userJJ', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiJ', 'Andropov'),
( 'userKK', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiK', 'Andropov'),
( 'userLL', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiL', 'Andropov'),
( 'userMM', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiM', 'Andropov'),
( 'userNN', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiN', 'Andropov'),
( 'userOO', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiO', 'Andropov'),
( 'userPP', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiP', 'Andropov'),
( 'userQQ', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiQ', 'Andropov'),
( 'userRR', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiR', 'Andropov'),
( 'userSS', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiS', 'Andropov'),
( 'userTT', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiT', 'Andropov'),
( 'userUU', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiU', 'Andropov'),
( 'userVV', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiV', 'Andropov'),
( 'userWW', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiW', 'Andropov'),
( 'userXX', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiX', 'Andropov'),
( 'userYY', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiY', 'Andropov'),
( 'userZZ', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiZ', 'Andropov'),
( 'userAAA', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiAAA', 'Androp'),
( 'userBBB', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiBBB', 'Androp'),
( 'userCCC', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiCCC', 'Androp'),
( 'userDDD', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiDDD', 'Androp'),
( 'userAAB', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiAAB', 'Androp'),
( 'userBBC', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiBBC', 'Androp'),
( 'userCCD', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiCCD', 'Androp'),
( 'userDDE', '5baa61e4c9b93f3f0682250b6cf8331b7ee68fd8', 'enrollee', 'AndreiDDE', 'Androp');

insert into application (user, faculty, average_mark, status) values
(3, 1 , 90, 'applied'),
(4, 1 , 20, 'applied'),
(5, 1 , 10, 'applied'),
(6, 1 , 15, 'applied'),
(7, 1 , 25, 'applied'),
(8, 1 , 50, 'applied'),
(9, 1 , 80, 'applied'),
(10, 1 , 90, 'applied'),
(11, 1 , 100, 'applied'),
(12, 1 , 70, 'applied'),
(13, 1 , 60, 'applied'),
(14, 1 , 20, 'applied'),
(15, 1 , 5, 'applied'),
(16, 1 , 6, 'applied'),
(17, 1 , 16, 'applied'),
(18, 1 , 27, 'applied'),
(19, 1 , 43, 'applied'),
(20, 1 , 89, 'applied'),
(21, 1 , 99, 'applied'),
(22, 1 , 100, 'applied'),
(23, 1 , 22, 'applied'),
(24, 1 , 2, 'applied'),
(25, 1 , 55, 'applied'),
(26, 1 , 75, 'applied'),
(27, 1 , 58, 'applied'),
(28, 1 , 50, 'applied'),
(29, 3 , 59, 'applied'),
(30, 3 , 56, 'applied'),
(31, 3 , 70, 'applied'),
(32, 3 , 80, 'applied'),
(33, 3 , 90, 'applied'),
(34, 3 , 10, 'applied'),
(35, 3 , 20, 'applied'),
(36, 3 , 30, 'applied'),
(37, 3 , 30, 'applied'),
(38, 3 , 80, 'applied'),
(39, 3 , 90, 'applied'),
(40, 3 , 50, 'applied'),
(41, 3 , 40, 'applied'),
(42, 3 , 50, 'applied'),
(43, 3 , 40, 'applied'),
(44, 3 , 50, 'applied'),
(45, 3 , 40, 'applied'),
(46, 3 , 50, 'applied'),
(47, 3 , 40, 'applied'),
(48, 3 , 90, 'applied'),
(49, 3 , 50, 'applied'),
(50, 3 , 90, 'applied'),
(51, 3 , 50, 'applied'),
(52, 3 , 90, 'applied'),
(53, 3 , 50, 'applied'),
(54, 3 , 90, 'applied'),
(55, 2 , 90, 'applied'),
(56, 2 , 80, 'applied'),
(57, 4 , 6, 'applied'),
(58, 4 , 5, 'applied'),
(59, 5 , 40, 'applied'),
(60, 5 , 59, 'applied'),
(61, 5 , 91, 'applied'),
(62, 5 , 11, 'applied');

insert into certificate (application, subject, mark) values
(1, 1, 50), (1, 2, 50), (1, 3, 50),
(2, 1, 50), (2, 2, 50), (2, 3, 50),
(3, 1, 50), (3, 2, 50), (3, 3, 50),
(4, 1, 50), (4, 2, 50), (4, 3, 50),
(5, 1, 50), (5, 2, 50), (5, 3, 50),
(6, 1, 50), (6, 2, 50), (6, 3, 50),
(7, 1, 50), (7, 2, 50), (7, 3, 50),
(8, 1, 50), (8, 2, 50), (8, 3, 50),
(9, 1, 50), (9, 2, 50), (9, 3, 50),
(10, 1, 50), (10, 2, 50), (10, 3, 50),
(11, 1, 50), (11, 2, 50), (11, 3, 50),
(12, 1, 50), (12, 2, 50), (12, 3, 50),
(13, 1, 50), (13, 2, 50), (13, 3, 50),
(14, 1, 50), (14, 2, 50), (14, 3, 50),
(15, 1, 50), (15, 2, 50), (15, 3, 50),
(16, 1, 50), (16, 2, 50), (16, 3, 50),
(17, 1, 50), (17, 2, 50), (17, 3, 50),
(18, 1, 50), (18, 2, 50), (18, 3, 50),
(19, 1, 50), (19, 2, 50), (19, 3, 50),
(20, 1, 50), (20, 2, 50), (20, 3, 50),
(21, 1, 50), (21, 2, 50), (21, 3, 50),
(22, 1, 50), (22, 2, 50), (22, 3, 50),
(23, 1, 50), (23, 2, 50), (23, 3, 50),
(24, 1, 50), (24, 2, 50), (24, 3, 50),
(25, 1, 50), (25, 2, 50), (25, 3, 50),
(26, 1, 50), (26, 2, 50), (26, 3, 50),
(27, 1, 50), (27, 2, 50), (27, 3, 50),
(28, 1, 50), (28, 2, 50), (28, 3, 50),
(29, 1, 50), (29, 2, 50), (29, 3, 50),
(30, 1, 50), (30, 2, 50), (30, 3, 50),
(31, 1, 50), (31, 2, 50), (31, 3, 50),
(32, 1, 50), (32, 2, 50), (32, 3, 50),
(33, 1, 50), (33, 2, 50), (33, 3, 50),
(34, 1, 50), (34, 2, 50), (34, 3, 50),
(35, 1, 50), (35, 2, 50), (35, 3, 50),
(36, 1, 50), (36, 2, 50), (36, 3, 50),
(37, 1, 50), (37, 2, 50), (37, 3, 50),
(38, 1, 50), (38, 2, 50), (38, 3, 50),
(39, 1, 50), (39, 2, 50), (39, 3, 50),
(40, 1, 50), (40, 2, 50), (40, 3, 50),
(41, 1, 50), (41, 2, 50), (41, 3, 50),
(42, 1, 50), (42, 2, 50), (42, 3, 50),
(43, 1, 50), (43, 2, 50), (43, 3, 50),
(44, 1, 50), (44, 2, 50), (44, 3, 50),
(45, 1, 50), (45, 2, 50), (45, 3, 50),
(46, 1, 50), (46, 2, 50), (46, 3, 50),
(47, 1, 50), (47, 2, 50), (47, 3, 50),
(48, 1, 50), (48, 2, 50), (48, 3, 50),
(49, 1, 50), (49, 2, 50), (49, 3, 50),
(50, 1, 50), (50, 2, 50), (50, 3, 50),
(51, 1, 50), (51, 2, 50), (51, 3, 50),
(52, 1, 50), (52, 2, 50), (52, 3, 50),
(53, 1, 50), (53, 4, 50), (53, 2, 50),
(54, 1, 50), (54, 4, 50), (54, 2, 50),
(55, 1, 50), (55, 4, 50), (55, 2, 50),
(56, 1, 50), (56, 4, 50), (56, 2, 50),
(57, 1, 50), (57, 5, 50), (57, 6, 50),
(58, 1, 50), (58, 5, 50), (58, 6, 50),
(59, 1, 50), (59, 5, 50), (59, 6, 50),
(60, 1, 50), (60, 5, 50), (60, 6, 50);

use university;

select subject.id, subject.name from faculty_subject inner join subject on faculty_subject.subject = subject.id where faculty_subject.faculty = 6;

select (application.average_mark + marks.sum) as amount from application 
inner join (select application, sum(mark) as sum from certificate group by application) marks on marks.application=application.id
 where application.faculty = 3 and application.status != 'cancelled';
 
 select COUNT(*) as count from application where faculty = 3 and status != 'cancelled';
 select * from application where user = 2;